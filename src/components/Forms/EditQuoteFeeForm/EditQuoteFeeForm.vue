        ...
        <main class="main">
            <div class="carriersFee container">
                <form method="POST" id="form" class="carriersFee__form">
                    ...
                    <input type="text" placeholder="Wrote the Quote ID Here..." class="carriersFee__form--inputSearch"
                        id="inpt_id" name="LineBusiness" v-model="inptId" />
                ...
                <div v-if="quote" class="carriersFee__tableContainer">
                    <table border="1" cellpadding="8" class="carriersFee__tableContainer--table">
                        <tr class="tr" v-for="(key, index) in Object.keys(quote[0])" :key="index">
                            <template v-if="index < 17">
                                <th class="carrierTh" v-if="quote[0][key] !== '' && quote[0][key] !== null">
                                    <!-- {{ key }} -->
                                    {{ formatAndCapitalize(key) }}
                                </th>
                                <td class="carrierTd" v-if="quote[0][key] !== '' && quote[0][key] !== null">
                                    {{ quote[0][key] }}
                                </td>
                            </template>
                        </tr>
                    ...
                    <div :if="accesorials">
                        <div class="carriersFee__tableContainer--carrirerFee">
                            <h2 class="feeTitle">Carrier Fee</h2>
                            ...
                            <div class="carrierAccesorialsContainer">
                                <div class="carrierAccesorialsContainer__col" v-for="item in accesorials" :key="item.id">
                                    ...
                                    <input v-if="accesorialSelected[item.accesorial]" class="accesorialValue" type="number"
                                        v-model="carrierAccesorialValues[item.accesorial]"
                                        @input="accesorialValuesOnChange" />
                                </div>
                            </div>
                        </div>
                        <!-- MAGNET FEE -->
                        <div class="carriersFee__tableContainer--magnetFee">
                            <h2 class="feeTitle">Magnet Fee</h2>
                            ...
                            <div class="magnetAccesorialContainer">
                                <div class="inptRow">
                                    <div class="inptCol">
                                        <label for="magnetFee">Magnet Fee</label>
                                        <input id="magnetFee" type="number" placeholder="Write down the answer here.."
                                            v-model="inptMagnetFee" @input="addingRates" />
                                    </div>

                                    <div class="inptCol">
                                        <label for="carrierChassis">Chassis Value</label>
                                        <input placeholder="Write down the answer here.." id="carrierChassis" type="number"
                                            v-model="inptMagnetChassis" @input="addigChassis" />
                                    </div>
                                </div>
                                <!-- ACCESORIAL -->
                                <h2 class="accesorialsTitle" v-if="Object.keys(accesorialSelected).length">Accesorials</h2>
                                <div class="magnetAccesorialsContainer">
                                    <div class="magnetAccesorialsContainer__col"
                                        v-for="(value, name, index) in accesorialSelected" :key="index">
                                        <label v-if="value">
                                            {{ name }}
...
const addingRates = () => {
  totalFee.value = parseFloat(inptCarrierFee.value) + parseFloat(inptMagnetFee.value);
};

const addigChassis = () => {
  totalChassis.value = inptCarrierChassis.value + inptMagnetChassis.value;
};
  ...
  getApi(`${import.meta.env.VITE_APP_API}/get/carriers-fees/${idValue}`)
    .then((data) => {
      if (Object.keys(data).length === 0) {
        showToast('This quote fee does not exist', 'danger', 'red')
        isLoading.value = false;
        return;
      }
      isLoading.value = false;
...
const formatAndCapitalize = (value) => {
  return value.replace(/([A-Z])/g, ' $1').trim().split(' ').map(word => word.charAt(0).toUpperCase() + word.slice(1)).join(' ');
};
...
const saveFee = async () => {
  ...
  quoteEdited.value.totalFee = totalFee.value;
  quoteEdited.value.totalChassis = parseFloat(inptCarrierChassis.value) + parseFloat(inptMagnetChassis.value);
  quoteEdited.value.notes = inptNotes.value;
 
  postApi(
    `${import.meta.env.VITE_APP_API}/post/update-carrier-fee`,
    quoteEdited.value
  )
    .then((data) => {
      if (data.message === "ok") {
